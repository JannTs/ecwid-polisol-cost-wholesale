...
export const PRICING_TABLE: Record<CanonLabel, [number, number, number, number, number]> = {
...
export function toCanonLabel(labelRaw: string): CanonLabel {
  ...
  for (const k of Object.keys(CONTENT_CANON)) {
    if (t.toLowerCase() === k.toLowerCase()) return CONTENT_CANON[k as keyof typeof CONTENT_CANON];
  }
